...
provider "kubernetes" {
  ...
  host                   = module.eks_user_cluster["enabled"].cluster_endpoint
  cluster_ca_certificate = base64decode(module.eks_user_cluster["enabled"].cluster_certificate_authority_data)
  exec {
    api_version = "client.authentication.k8s.io/v1beta1"
    args        = ["eks", "get-token", "--cluster-name", module.eks_user_cluster["enabled"].cluster_name]
    command     = "aws"
  }
}
